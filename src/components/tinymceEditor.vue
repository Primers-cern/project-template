...
<script setup>
import { defineProps, defineEmit, ref, watch, watchEffect, inject } from "vue";
...
const props = defineProps({
  ...
  toolbar: {
    type: [String, Array],
    default: "fontsizeselect forecolor backcolor | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify  | numlist bullist | link image | fullscreen",
  },
})

const contentValue = ref('')
watchEffect(() => contentValue.value = props.value)
watch(contentValue, val => emit('input', val))
...
const init = {
  ...
  plugins: ['image', 'fullscreen', 'link', 'autoresize'],  //插件配置
  ...
  images_upload_handler: (blobInfo, success, failure) => {
    if (blobInfo.blob().size / 1024 / 1024 > 2) return failure("上传失败，图片大小请控制在 2M 以内")
...
const clear = () => contentValue.value = ''
</script>
